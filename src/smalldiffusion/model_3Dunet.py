...
def Upsample(ch):
    return nn.Sequential(
        ...
        torch.nn.Conv3d(ch, ch, kernel_size=3, stride=1, padding=1),
    )

def Downsample(ch):
    return nn.Sequential(
        nn.ConstantPad3d((0, 1, 0, 1, 0, 1), 0),
        torch.nn.Conv3d(ch, ch, kernel_size=3, stride=2, padding=0),
    )
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_ch, out_ch=None, conv_shortcut=False,
        ...
        self.layer1 = nn.Sequential(
            Normalize(in_ch),
            nn.SiLU(),
            torch.nn.Conv3d(in_ch, out_ch, kernel_size=3, stride=1, padding=1),
        )
        ...
        self.layer2 = nn.Sequential(
            ...
            torch.nn.Conv3d(out_ch, out_ch, kernel_size=3, stride=1, padding=1),
        )
        if self.in_ch != self.out_ch:
            kernel_stride_padding = (3,1,1) if self.use_conv_shortcut else (1,1,0)
            self.shortcut = torch.nn.Conv3d(in_ch, out_ch, *kernel_stride_padding)

    def forward(self, x, temb):
        h = x
        h = self.layer1(h)
        h = h + self.temb_proj(temb)[:, :, None, None, None] # (B, C, x, y, z)
        h = self.layer2(h)
        if self.in_ch != self.out_ch:
            x = self.shortcut(x)
        return x + h
...
class AttnBlock(nn.Module):
    def __init__(self, ch, num_heads=1):
        ...
        self.proj_out = nn.Conv3d(ch, ch, kernel_size=1, stride=1, padding=0)

    def forward(self, x, temb):
        # temb is currently not used, but included for CondSequential to work
        B, C, X, Y, Z = x.shape
        h_ = self.norm(x)
        h_ = rearrange(h_, 'b c x y z -> b (x y z) c')
        h_ = self.attn(h_)
        h_ = rearrange(h_, 'b (x y z) c -> b c x y z', x=X, y=Y, z=Z)
        return x + self.proj_out(h_)

class Unet3D(nn.Module, ModelMixin):
    def __init__(self, in_dim, in_ch, out_ch,
        ...
        self.input_dims = (in_ch, in_dim, in_dim, in_dim)
        ...
        self.conv_in = torch.nn.Conv3d(in_ch, self.ch, kernel_size=3, stride=1, padding=1)
        ...
        self.out_layer = nn.Sequential(
            Normalize(block_in),
            nn.SiLU(),
            torch.nn.Conv3d(block_in, out_ch, kernel_size=3, stride=1, padding=1),
        )

    def forward(self, x, sigma, cond=None):
        assert x.shape[2] == x.shape[3] == x.shape[4] == self.in_dim
        # Embeddings
